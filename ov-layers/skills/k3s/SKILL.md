...
dependent layers `/ov-layers:k3s-server` and `/ov-layers:k3s-agent`,
which emit systemd units that wrap this binary with the right CLI verb
(`k3s server` vs `k3s agent`).

## Usage

Typically not used directly — compose `/ov-layers:k3s-server` or
`/ov-layers:k3s-agent` (both depend on this layer).
...
## Related Layers
- `/ov-layers:k3s-server` — Control-plane node (depends on this layer)
- `/ov-layers:k3s-agent` — Worker node (depends on this layer)
- `/ov-layers:kubernetes` — Distro `kubectl`/`helm` binaries for the operator, not the cluster
