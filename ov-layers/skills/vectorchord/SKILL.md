...
```yaml
# images.yml -- used alongside postgresql for Immich smart search
my-image:
  layers:
    - postgresql
    - vectorchord
...
- `/ov-layers:postgresql` -- required dependency (provides PostgreSQL server and pgvector)
- `/ov-layers:immich` -- primary consumer (migration script creates the extension)
