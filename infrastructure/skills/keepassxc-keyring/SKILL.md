...
| Skill | Relationship |
|-------|--------------|
| `/ov-infrastructure:keepassxc` | The package-only layer. `keepassxc-keyring` `requires:` on it; never composes it. |
| `/ov-infrastructure:gnupg` | Same — keepassxc-keyring `requires:` on gnupg, never composes. |
