...
- `/ov-core:shell` — `ov shell` for one-shot commands (no persistence) or `ov shell -c` (full container setup)
- `/ov-eval:cdp` — Chrome DevTools Protocol (used with tmux for OAuth flows)
- `/ov-automation:openclaw-deploy` — OpenClaw gateway config (OAuth requires tmux for token exchange)
- `/ov-core:service` — Supervisord service management (different scope: persistent services vs ad-hoc commands)
- `/ov-infrastructure:tmux-layer` — The tmux layer definition
- `/ov-infrastructure:dbus-layer` — D-Bus session bus (required for notifications)
