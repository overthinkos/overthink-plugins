...
Pixelflux is compiled **from source** in the selkies build stage (`layers/selkies/build.sh`), not installed from PyPI, because these fixes live on a fork not yet merged upstream. The build stage clones from a pinned commit, applies four inline source patches, and runs `pip install .` against the fedora-builder image. See `/ov-foundation:fedora-builder` (rpmfusion is applied first so codec devel libs install) and `/ov-coder:build-toolchain` (5 package categories — smithay backend headers, codec devel, bindgen runtime, rust, generic C/C++) for the builder-stage dependency story.
...
- `/ov-foundation:supervisord` — Service ordering, event listeners, crash-loop escalation
...
- `/ov-foundation:nvidia`, `/ov-foundation:rocm` — GPU runtime layers feeding DRINODE into the selkies VAAPI encoder
...
- `/ov-foundation:rpmfusion` — Applied before build-toolchain so codec devel libs (libva-devel, x264-devel, ffmpeg-devel) install

## Related Images

- `/ov-selkies:selkies-desktop`, `/ov-selkies:selkies-desktop-nvidia` — Images that bundle this layer
- `/ov-foundation:fedora-builder` — Builder image for the pixelflux from-source compilation

## Related Commands

- `/ov-advanced:wl` — Wayland automation (screenshot via capture bridge, input, windows)
- `/ov-advanced:cdp` — Chrome DevTools and SPA bridge (click, type, key-combo through remote desktop)
- `/ov-advanced:record` — Desktop video recording via capture bridge
