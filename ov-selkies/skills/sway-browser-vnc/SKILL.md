...
- `/ov-selkies:sway-desktop-vnc`, `/ov-selkies:sway`, `/ov-selkies:wayvnc`,
  ...
  `/ov-foundation:ov`, `/ov-foundation:agent-forwarding`
- `/ov-build:eval` — declarative testing framework (parent router for `ov eval cdp|wl|dbus|vnc|mcp`)
- `/ov-advanced:vnc` — VNC automation on this image
