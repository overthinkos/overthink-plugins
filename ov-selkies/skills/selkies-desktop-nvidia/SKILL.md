...
- `/ov-foundation:nvidia` — parent base image with CUDA toolkit

## Related Commands
- `/ov-build:eval` — parent router for live-container verbs (`ov eval cdp|wl|dbus|vnc|mcp`)
- `/ov-advanced:cdp` — drive Chrome inside the streaming desktop
- `/ov-advanced:wl` — interact with the labwc Wayland session
