...
Ubuntu 24.04 (noble) counterpart of `/ov-images:fedora-builder` and `/ov-images:debian-builder`. Same role — pixi/npm/cargo multi-stage builder — with one important difference: the builder runs as `ubuntu` (uid 1000) because the upstream `ubuntu:24.04` base image ships a pre-existing `ubuntu:ubuntu` account at uid 1000, and `build.yml distro.ubuntu.base_user` adopts it.
...
| User | `ubuntu` / uid 1000 (**adopt mode** — see `/ov:image` "user_policy") |
| Home | `/home/ubuntu` |

## Full layer stack

1. `/ov-images:ubuntu` — Ubuntu 24.04 + bootstrap. Inherits Debian's `apt-get update && apt-get install -y --no-install-recommends curl ca-certificates gnupg` pattern because `build.yml distro.ubuntu` declares `inherits: debian`. Ubuntu-specific: `base_user: { name: ubuntu, uid: 1000, gid: 1000, home: /home/ubuntu }` — no `useradd` step emitted.
2. `/ov-layers:pixi` — pixi package manager + env paths (`/home/ubuntu/.pixi`).
3. `/ov-layers:nodejs` — Node.js + npm (generic `nodejs`, not `nodejs24`).
4. `/ov-layers:build-toolchain` — same Debian `-dev` packages as `/ov-images:debian-builder`.
...
No `useradd`, no `groupadd`, no `usermod -l` rename. The upstream `ubuntu:ubuntu` account is honored verbatim — HOME, npm prefix, pixi env, cargo home, sudoers all derive from `resolved.User = "ubuntu"`. See `/ov:image` "user_policy" and `/ov-dev:generate` "writeBootstrap".
...
During `ov image build ubuntu-coder`, cargo/npm/pixi-owning layers get their `FROM ubuntu-builder AS <layer>-<type>-build` stages from this image, then `COPY --from=<stage> --chown=1000:1000 /home/ubuntu /home/ubuntu` into the final ubuntu-coder. (The `--chown=1000:1000` numeric form works uniformly regardless of user name — see `/ov-layers:build-toolchain` for the builder-artifact COPY pattern.)

## Cross-distro sibling builders

- `/ov-images:fedora-builder` — RPM-family, `user:user` uid 1000 (create).
- `/ov-images:debian-builder` — deb-family, Debian 13, `user:user` (create — Debian 13 ships no pre-existing uid-1000 user).
- `/ov-images:archlinux-builder` — pacman-family, `user:user` + `yay` for AUR.
...
Typically not invoked directly — it's a build-time dependency of `/ov-images:ubuntu-coder`.
...
- `/ov-images:ubuntu-coder` — the consumer that this builder serves.
- `/ov-images:debian-builder` — deb-family sibling without adopt mode.
- `/ov-images:fedora-builder` — canonical RPM-family sibling.
...
- `/ov:build` — `base_user:` declaration format in `build.yml distro.*`
- `/ov:image` — `user_policy:` field (auto / adopt / create) and the decision table
- `/ov:generate` — adopt-vs-create writeBootstrap emission modes
