...
Resolved identity: `User=user, UID=1000, GID=1000, Home=/home/user`. Identical to fedora-coder / arch-coder. Contrast with `/ov-images:ubuntu-coder` which runs adopt mode on `ubuntu:ubuntu`.

See `/ov:image` "user_policy" and `/ov:build` "base_user" for the full decision table.
...
| sudo | passwordless via `/etc/sudoers.d/ov-user` (discovered via `getent passwd 1000`) | `/ov-layers:sshd` |

## Debian-specific layer quirks

- **bat → batcat symlink** (`/ov-layers:dev-tools`). Debian and Ubuntu rename `bat` → `batcat` to avoid a namespace collision with a legacy `bacula` utility. The dev-tools layer ships a distro-tolerant cmd task that creates `/usr/bin/bat -> /usr/bin/batcat` when only batcat is present; no-op on Fedora/Arch.
- **dotnet-sdk-9.0 via Microsoft's `dotnet-install.sh`** (`/ov-layers:language-runtimes`). Debian 13 main ships no .NET; Microsoft's trixie apt repo has dotnet-sdk-9.0, but using the official cross-distro `dotnet-install.sh` channel-pin to `9.0` gives version parity with Ubuntu (whose Microsoft noble repo only ships 10.0). Installs to `/usr/share/dotnet` + symlinks `/usr/bin/dotnet`.
- **libvirt tests use `package:` not `file:`** (`/ov-layers:virtualization`). Debian bundles libvirt drivers differently from Fedora's split packaging, so `file: /usr/sbin/virtqemud` would false-fail. The layer now probes via `package: libvirt-daemon-driver-qemu` + `package_map:` per-distro package name.
- **sudoers via `getent`** (`/ov-layers:sshd`). Instead of hardcoding `user ALL=…`, the layer runs `getent passwd 1000 | cut -d: -f1` at build time to discover the actual uid-1000 account. On debian-coder that returns `user`; on ubuntu-coder it returns `ubuntu`. One cross-distro implementation, no special cases.
...
- `/ov-images:fedora-coder` — RPM (Fedora 43 via fedora-nonfree).
- `/ov-images:arch-coder` — pacman + optional AUR.
- `/ov-images:ubuntu-coder` — deb on Ubuntu 24.04; **adopt mode** (`user:ubuntu`).
- `/ov-images:debian-coder` — this image; create mode (`user:user`).

## Related images

- `/ov-images:debian` — parent base.
- `/ov-images:debian-builder` — multi-stage builder for pixi/npm/cargo.

## Related layers (Debian-specific content)

- `/ov-layers:sshd` — cross-distro sudoers via `getent passwd 1000`.
- `/ov-layers:dev-tools` — bat → batcat symlink task.
- `/ov-layers:language-runtimes` — Microsoft `dotnet-install.sh` cross-distro pattern.
- `/ov-layers:virtualization` — `package:` + `package_map:` pattern for libvirt.
- `/ov-layers:build-toolchain` — Debian `-dev` package equivalents of Fedora's `-devel`.
- `/ov-layers:gh`, `/ov-layers:docker-ce`, `/ov-layers:kubernetes`, `/ov-layers:container-nesting` — each adds an upstream apt repo (with GPG key) for packages not in Debian main.
