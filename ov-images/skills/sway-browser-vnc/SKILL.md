...
ov test vnc status sway-browser-vnc
ov test wl screenshot sway-browser-vnc screenshot.png
```

## D-Bus and Notification Support

This image includes `dbus` and `ov` layers, enabling:
- `ov test dbus notify` — native Go D-Bus notifications via in-container ov binary
- `ov test dbus list/call/introspect` — full D-Bus interaction
...
ov test dbus list sway-browser-vnc          # List all D-Bus services
```
...
```bash
ov test wl overlay show sway-browser-vnc --type text --text "INTRO" --name title
ov test wl overlay show sway-browser-vnc --type lower-third --text "Speaker" --subtitle "Role" --name lt
ov test wl overlay show sway-browser-vnc --type countdown --seconds 3 --name cd
ov test wl overlay show sway-browser-vnc --type highlight --region "430,290,510,50" --name hl
ov test wl overlay show sway-browser-vnc --type fade --color black --name outro
ov test wl overlay hide sway-browser-vnc --all
```
...
ov test cdp open sway-browser-vnc "https://ov-selkies-desktop:3000"

# Interact with the remote desktop:
# Mouse: CDP Input.dispatchMouseEvent on the Selkies tab (with ~0.82x coordinate scaling)
# Keyboard: ov test vnc type sway-browser-vnc "text" (passthrough via SPA's overlayInput)
# Screenshots: ov test vnc screenshot sway-browser-vnc (shows full client desktop with stream)
#              ov test cdp screenshot sway-browser-vnc $TAB (shows only the stream content)
```
...
- Clipboard permission dialog appears on first connection — dismiss with `ov test vnc key Return` or CDP `Browser.grantPermissions`
...
- `/ov:test` — declarative testing framework (parent router for `ov test cdp|wl|dbus|vnc|mcp`)
