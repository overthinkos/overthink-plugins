...
supervisorctl internally) for program-liveness checks. See `/ov-build:eval`
Authoring Gotcha #3.
...
On non-bootc images, supervisord is container PID 1 (`ENTRYPOINT=supervisord` emitted by the init system definition in `build.yml`). On **bootc** images, systemd is PID 1, which means supervisord needs a systemd unit wrapper — otherwise the whole desktop tier never starts. `/ov-foundation:bootc-config` ships that wrapper as a systemd **user** unit at `/etc/systemd/user/supervisord.service`, `systemctl --global enable`d so tty1 autologin brings it up. See `/ov-foundation:bootc-config` for the full mechanism (including the linger sentinel file).
...
- `/ov-foundation:traefik` -- Reverse proxy (depends on supervisord)
...
- `/ov-ollama:ollama`, `/ov-openclaw:openclaw`, `/ov-foundation:postgresql`, `/ov-foundation:redis`, `/ov-selkies:sway`, `/ov-selkies:mutter`, `/ov-selkies:niri`, `/ov-selkies:kwin` -- All ship `service:` blocks
...
- `/ov-build:layer` — `service:` field authoring + `security:` resource caps that drive the circuit breaker
