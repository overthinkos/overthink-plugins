...
- No `security_opt` relaxation beyond what `/ov-foundation:container-nesting` contributes (and even that isn't needed for VMs alone).
...
See `/ov-build:eval` "`package:` + `package_map:` pattern" for the resolution order (tag > base name > fallback).
...
- `/ov-foundation:fedora-ov` — root VM host (same daemons, uid 0)
- `/ov-coder:arch-ov` — Arch counterpart
- `/ov-coder:debian-coder`, `/ov-coder:ubuntu-coder` — deb-based consumers (via `ov-full`)
- `/ov-foundation:githubrunner` — VMs for CI workloads
- `/ov-foundation:aurora`, `/ov-foundation:bazzite-ai` — bootc siblings (currently `enabled: false`)

## Related Layers

- `/ov-coder:ov-full` — composition that pulls this layer into ov-toolchain images
- `/ov-foundation:container-nesting` — pairs with this layer for images that need both nested containers AND nested VMs; also donates `/dev/kvm`-adjacent devices
- `/ov-foundation:socat` — part of `ov-full` alongside virtualization; used for VM console/hostfwd relays
- `/ov-foundation:gocryptfs` — part of `ov-full`; for encrypting VM disk storage
