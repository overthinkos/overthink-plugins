...
- `/ov-foundation:nvidia` — NVIDIA GPU runtime (driver libs, CDI generation via nvidia-ctk)
- `/ov-foundation:cuda` — CUDA toolkit and GPU development libraries
...
- `/ov-foundation:fedora` — parent base (no GPU)
- `/ov-coder:arch-ov` — Arch Linux ov toolchain with nvidia (shared layers)
- `/ov-foundation:fedora-ov` — Fedora ov toolchain with nvidia (shared layers)
