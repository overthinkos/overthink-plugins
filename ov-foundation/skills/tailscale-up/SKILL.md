...
description: |
  ...
  with the system hostname. Depends on /ov-foundation:tailscale (which
...
1. **Starts `tailscaled.service`** via `systemctl start ... || true`.
   The `/ov-foundation:tailscale` layer is `systemctl enable`-only
...
3. Resolves the human deploy user via `getent passwd 1000 | cut -d: -f1`.
   This matches `/ov-foundation:wheel-nopasswd` and the
   `renderAurScript` helper in `ov/deploy_target_local.go` — uid 1000
...
- Bootc images — they need fresh-boot semantics that don't match the
  deploy-runtime contract here. Use `/ov-foundation:tailscale` alone.
...
matrix and `/ov-advanced:sidecar` for the tailscale-sidecar pattern
(an alternative when a pod needs its own tailnet identity, not just
host-level serve).

## Related Skills

- `/ov-foundation:tailscale` — the install-side companion (daemon
...
- `/ov-advanced:local-deploy` — the `target: local` execution model
  this layer is designed for.
- `/ov-build:local-spec` — `local.yml` template authoring; the
  canonical consumer is `local.ov-cachyos`.
- `/ov-foundation:wheel-nopasswd` — provides the passwordless sudo
  used by the eval probe's `tailscale debug prefs` chain.
- `/ov-build:layer` — layer authoring reference.
- `/ov-build:eval` — declarative testing reference (three-state
  pattern used here).
