...
See `/ov-build:layer` for `env_provides` field docs.
...
- `redis` (dnf request) → installed as **`valkey-compat-redis`** on
  ...
  `/ov-build:eval` Authoring Gotcha #8.
...
OCI label (see `/ov-build:eval` for the full schema — this layer is the
...
- `/ov-foundation:postgresql` -- often paired with redis in service stacks
- `/ov-build:eval` -- declarative testing framework (this layer is the gold-standard pattern)
- `/ov-build:layer` -- layer authoring + `env_provides` field docs
- `/ov-foundation:valkey` -- Remi-repo Valkey 9 package (separate layer; different version than Fedora's default valkey-compat-redis)
