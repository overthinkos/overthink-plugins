...
Base Debian 13 (trixie) image. Root of the deb-family image hierarchy alongside `/ov-foundation:ubuntu`.
...
This is the intentional asymmetry vs `/ov-foundation:ubuntu`, which DOES ship `ubuntu:ubuntu` at uid 1000 and declares `base_user:` to adopt that identity. See `/ov-build:image` "user_policy" and `/ov-build:build` "base_user" for the full decision table.
...
Currently the single consumer is `/ov-foundation:debian-builder` (itself the builder for `/ov-coder:debian-coder`).
...
- `/ov-foundation:ubuntu` — sibling deb-family base; ships `base_user:` + adopts ubuntu:ubuntu.
- `/ov-foundation:debian-builder` — multi-stage builder on top of this image.
...
- `/ov-build:image` — `user_policy:` reconciliation.
