...
- `/ov-foundation:nvidia` — NVIDIA GPU base image (nvidia + cuda layers)
- `/ov-coder:arch-ov` — Arch Linux ov toolchain (shared layers + nvidia)
- `/ov-foundation:fedora-ov` — Fedora ov toolchain (shared layers + nvidia)

## Related Layers

- `/ov-foundation:cuda` — CUDA development toolkit (depends on nvidia)
- `/ov-foundation:rocm` — AMD GPU counterpart (ROCm runtime + OpenCL), uses the same `appendAutoDetectedEnv()` DRINODE injection
- `/ov-selkies:selkies` — Primary consumer of the DRINODE env for VAAPI H.264 encode
- `/ov-foundation:python-ml`, `/ov-jupyter:llama-cpp`, `/ov-jupyter:jupyter-ml` — CUDA ML stacks that depend on this layer
...
- `/ov-advanced:udev` — Device permission management for `/dev/dri/*` and `/dev/nvidia*`
