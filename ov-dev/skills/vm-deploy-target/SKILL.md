...
5. **Walk plans.** Same batched `(Scope, Venue)` logic as `HostDeployTarget`, but with `sudo bash -s` wrapped in `ssh`. See `/ov-advanced:local-deploy` "HostDeployTarget execution model" for the grouping rules.
...
- `/ov-advanced:local-deploy` — parallel target (HostDeployTarget); ReverseOps model also used on VM target
