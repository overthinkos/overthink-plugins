...
- `/ov:mcp` — Author-facing reference for the standalone `ov test mcp` verb: method catalog, URL-rewrite behavior, port-publishing gotcha, transport dispatch. Pair with the file table's `mcp.go` + `mcp_client.go` rows above.
