...
- `/ov-foundation:postgresql` -- database dependency
- `/ov-foundation:redis` -- cache dependency
- `/ov-selkies:ffmpeg` -- FFmpeg multimedia (nonfree codecs) — required dependency
- `/ov-foundation:vectorchord` -- VectorChord extension (migration script creates it if available)
