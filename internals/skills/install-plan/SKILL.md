...
| `LocalPkgInstallStep` | PkgbuildRef, LayerName, LayerDir, ProjectDir, Format, LocalPkg (`*LocalPkgDef`), BuilderImage, DepBuilderDef | HostNative | Always system. Compiled from a layer's `localpkg: <dir>` field at "step 2.5" (before tasks); `Format` + `LocalPkg` resolve from the target distro's `format.<fmt>.local_pkg:` block in `build.yml` (`DistroDef.LocalPkgFormat`), so EVERY package-manager command is rendered from config — no hardcoded makepkg/pacman/glob literals. On a localpkg-capable deploy target (a pac host via `target: local` / a pac guest via `target: vm`) the HOST builds the referenced PKGBUILD via `LocalPkg.BuildTemplate` (makepkg → `LocalPkg.PkgGlob` artifacts), then resolves the built package's **builder-resolvable dependency closure** — its `.PKGINFO depends` ∩ host-foreign packages (`LocalPkg.ForeignQuery` = `pacman -Qmq`) — and builds that closure through `LocalPkg.DepBuilder` (the `aur` builder; image in `BuilderImage`, def in `DepBuilderDef`), then installs the package + its closure onto the target via `LocalPkg.InstallTemplate` (`pacman -U`) through the SHARED transfer+install leg the aur builder also uses (R3). `LocalPkg.Probe` gates the install leg. `resolveLocalPkgDir` walks up from `ProjectDir`, so a consumer nested under `image/<distro>` finds the superproject `pkg/arch`. Skipped at image build (`OCITarget`) + on targets whose distro declares no localpkg-capable format (the layer's own curl/COPY task is the fallback there). Machinery: `ov/localpkg.go`; config: `build.yml format.<fmt>.local_pkg:`. |
