...
**Build-emit externalization (C1.2) — the HOST-COUPLED `SystemPackagesStep`.** The same
build-emit-only pattern extends to the first HOST-COUPLED kind: `SystemPackagesStep` keeps its typed
struct + methods + native compile (`compileSystemPackageSteps`) and its UNCHANGED deploy leg
(host-engine via `RunHostStep` → `renderHostPackageCommand`), but its pod-overlay BUILD-emit moved off
`OCITarget` into `candy/plugin-installstep` (`pluginEmitStepWords[SystemPackages] = "system-packages"`).
UNLIKE the seven PURE kinds, its render needs the host build ENGINE (the `DistroDef` format templates +
`RenderTemplate`), which cannot cross the process boundary — so the plugin's `OpEmit` calls back the host
`step-emit` host-builder (`Executor.HostBuild`) and echoes the returned `EmitReply`; the in-core render
(`stepEmitSystemPackages`, `step_emit_hostbuild.go`) resolves the box FormatDef via `DistroCfg.FindFormat`
and renders the `phase.install.container` template — byte-equivalent to the former in-proc build-emit.
...
Each step's `Reverse()` emits typed `ReverseOp` values. Adding a step kind means: (a) define the struct in `install_plan.go`, (b) decide its Scope/Venue/Gate/Reverse, (c) wire its BUILD-emit through a `class:step` plugin `OpEmit` referenced from `pluginEmitStepWords` (`candy/plugin-installstep`) — a PURE kind renders the fragment directly from the step VIEW (the C1.1 pattern), a HOST-COUPLED kind (whose render needs the host build engine) instead calls back `HostBuild("step-emit", …)` and registers an in-core `stepEmitter` via `registerStepEmitter` (the C1.2 pattern, `stepEmitSystemPackages`); a still-in-proc host-engine kind keeps an in-proc `StepProvider.EmitOCI` — plus its DEPLOY leg in the shared `charly/plugin/kit.WalkPlans` (and `RunHostStep` if it is a host-engine kind), (d) ensure the compiler in `install_build.go` emits it.
