...
| `Tests` | `LabelEval` | Three-section `{layer, image, deploy}` JSON — the tests baked into the image, consumed by `charly eval live` / `charly eval box`. See `/charly-eval:eval`. |
| `Shell` | `LabelShell` | Three-section `{layer, image, deploy}` JSON shell-init manifest. Each entry carries an Origin (layer name / "image" / "deploy"), an ID for overlay keying, an optional Generic body (intrinsic init + path_append) and a per-shell ByShell map (bash/zsh/fish/sh sub-blocks). Consumed by `charly box inspect`, `charly deploy from-box`, and `MergeDeployShell` for deploy.yml `shell:` overlay merging. See `/charly-image:layer` "Shell Init Surface". |
