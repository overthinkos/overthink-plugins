...
**What is permitted / required.** Building the real composition, running the bed, inspecting the running deployment, reading the emitted artifact, or a focused `ov eval box` / `ov eval live` probe — riskiest-assumption first — to PROVE a high-risk claim before editing on it. When the bed contradicts a skill or CLAUDE.md, the DOC IS STALE — fix it in the same change (skills are living documents).
...
## ADD. Agent Driven Development — the spec is the test; agents author and grade it

**The rule.** Capture an entity's intended behaviour as executable Gherkin scenarios on the LAYER that provides it (a `description:` block — Feature / Narrative + Given/When/Then), authored by you OR an agent (`ov candy add-scenario`). Each step binds to its verifier BY SHAPE: embed a check verb → a DETERMINISTIC check the runner executes; leave the step prose-only → an AGENT grades it (`ov eval feature run <deployment>` spawns the configured `kind: ai` CLI, which probes the live deployment and returns a pass/fail verdict with evidence). The scenarios bake into `org.overthinkos.description` and are runnable as acceptance tests (`ov box feature run <image>` / `ov eval feature run <deployment>`), drivable to green by hand OR by the `ov eval run <score>` AI loop. The spec IS the test.

**Risk — and the BIND choice — are author decisions, not afterthoughts.** A behaviour that can be checked mechanically (a file exists, a port answers, an MCP tool is listed) MUST embed the check verb — deterministic, reproducible, free. Reserve the agent grader for genuinely free-form behaviour a verb can't express ("the chart looks populated", "the page recovered gracefully"). Authoring a prose step where a deterministic check would do is the over-engineering twin of an unvalidated assumption — it trades a free, reliable check for a slow, costly, non-deterministic agent call.

**Forbidden internal-voice triggers.** "the scenario passed, no need to look at the evidence" / "the grader is non-deterministic so I'll just rerun until it's green" (that's R1/R4 — investigate, don't reroll) / "I'll mark every step prose and let the agent figure it out" (over-uses the grader; embed the verb) / "an unparseable grader output is probably a pass" (NEVER — unparseable / timed-out / launch-failed is a FAIL) / "I'll bake a failing scenario as a TODO" (a baked scenario must PASS — a known-failing one poisons every `ov eval run`).

**What is permitted / required.** Embedding check verbs for the measurable; the agent grader for the genuinely free-form, with its evidence inspected; `ov feature pending` to find still-prose steps; `--no-agent` for a deterministic-only CI run (prose reports `unbound`, visibly, never silently green). A scenario authored on a layer covers every image that composes it — author ONCE on the behaviour's provider, never per-image (R3).

**Why it matters.** "Done" without an executable definition of correct behaviour is the ambiguity ADD removes; a `then:` that documents intent but verifies nothing is prose that never runs — the grader makes it executable. And scenarios on the LAYER, not the image, keep one behaviour's acceptance in one place.

**Interaction with other rules.** ADD is the acceptance/intent layer of the same *never trust, verify* family: RDD proves the risky ASSUMPTIONS before the edit, ADD specifies WHAT correct behaviour is, R10 proves it on a fresh rebuild. It is a co-equal pillar and an OPT-IN runnable gate — never a blocking mandate to author scenarios (where an entity authors them they run and must pass; where it authors none, nothing is forced). The grader runs inside the secured, disposable box (candyboxing). Canonical definition in CLAUDE.md "Agent Driven Development (ADD)"; the how-to + the worked example live in `/ov-eval:eval`.
...
**Forbidden patterns.** Sibling-layer naming (`<name>-host`, `<name>-pod`, `<name>-bootc` when they share content); parallel filter functions in adjacent files; per-call-site re-implementations of the same predicate; copy-pasted YAML stanzas across multiple candy.yml files; copy-pasted Go function bodies with one-token differences; "let me just patch this one consumer for now and unify later"; "the abstraction is unclear, so I'll duplicate for now and refactor when the pattern firms up".
