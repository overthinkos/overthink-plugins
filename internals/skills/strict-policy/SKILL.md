...
**The rule.** Capture an entity's intended behaviour as executable Gherkin scenarios on the CANDY that provides it (a `description:` block — Feature / Narrative + Given/When/Then), authored by you OR an agent (`charly candy add-scenario`). Each step binds to its verifier BY SHAPE: embed a check verb → a DETERMINISTIC check the runner executes; leave the step prose-only → an AGENT grades it (`charly check feature run <deployment>` spawns the configured `kind: ai` CLI, which probes the live deployment and returns a pass/fail verdict with evidence). The scenarios bake into `ai.opencharly.description` and are runnable as acceptance tests (`charly box feature run <image>` / `charly check feature run <deployment>`), drivable to green by hand OR by the `charly check run <score>` AI loop. The spec IS the test.
...
**Interaction with other rules.** ADE is the acceptance/intent layer of the same *never trust, verify* family: RDD proves the risky ASSUMPTIONS before the edit, ADE specifies WHAT correct behaviour is, R10 proves it on a fresh rebuild. It is a co-equal pillar and a MANDATORY gate for candies: every candy MUST author a full `description:` (a non-empty `feature:` + ≥1 `scenario:`) AND a non-empty `check:` — `charly box validate` hard-errors otherwise — and the baked scenarios run and must pass. (Authoring is required; running the live AGENT GRADER via `charly check feature run` stays opt-in.) The grader runs inside the secured, disposable box (candyboxing). Canonical definition in CLAUDE.md "Agent Driven Evaluation (ADE)"; the how-to + the worked example live in `/charly-check:check`.
