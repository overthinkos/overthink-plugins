...
**What is permitted / required.** Embedding check verbs for the measurable; the agent grader for the genuinely free-form, with its evidence inspected; `charly feature pending` to find still-prose steps; `--no-agent` for a deterministic-only CI run (prose reports `unbound`, visibly, never silently green). A scenario authored on a layer covers every image that composes it — author ONCE on the behaviour's provider, never per-image (R3).
