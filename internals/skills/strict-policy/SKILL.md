...
**The rule.** Capture an entity's intended behaviour as executable Gherkin scenarios on the CANDY that provides it (a `description:` block — Feature / Narrative + Given/When/Then), authored by you OR an agent (`charly candy add-scenario`). Each step binds to its verifier BY SHAPE: embed a check verb → a DETERMINISTIC check the runner executes; leave the step prose-only → an AGENT grades it (`charly eval feature run <deployment>` spawns the configured `kind: ai` CLI, which probes the live deployment and returns a pass/fail verdict with evidence). The scenarios bake into `ai.opencharly.description` and are runnable as acceptance tests (`charly box feature run <image>` / `charly eval feature run <deployment>`), drivable to green by hand OR by the `charly eval run <score>` AI loop. The spec IS the test.
...
**Forbidden internal-voice triggers.** "the scenario passed, no need to look at the evidence" / "the grader is non-deterministic so I'll just rerun until it's green" (that's R1/R4 — investigate, don't reroll) / "I'll mark every step prose and let the agent figure it out" (over-uses the grader; embed the verb) / "an unparseable grader output is probably a pass" (NEVER — unparseable / timed-out / launch-failed is a FAIL) / "I'll bake a failing scenario as a TODO" (a baked scenario must PASS — a known-failing one poisons every `charly eval run`).

**What is permitted / required.** Embedding check verbs for the measurable; the agent grader for the genuinely free-form, with its evidence inspected; `charly feature pending` to find still-prose steps; `--no-agent` for a deterministic-only CI run (prose reports `unbound`, visibly, never silently green). A scenario authored on a candy covers every box that composes it — author ONCE on the behaviour's provider, never per-box (R3).

**Why it matters.** "Done" without an executable definition of correct behaviour is the ambiguity ADE removes; a `then:` that documents intent but verifies nothing is prose that never runs — the grader makes it executable. And scenarios on the CANDY, not the box, keep one behaviour's acceptance in one place.

**Interaction with other rules.** ADE is the acceptance/intent layer of the same *never trust, verify* family: RDD proves the risky ASSUMPTIONS before the edit, ADE specifies WHAT correct behaviour is, R10 proves it on a fresh rebuild. It is a co-equal pillar and a MANDATORY gate for candies: every candy MUST author a full `description:` (a non-empty `feature:` + ≥1 `scenario:`) AND a non-empty `eval:` — `charly box validate` hard-errors otherwise — and the baked scenarios run and must pass. (Authoring is required; running the live AGENT GRADER via `charly eval feature run` stays opt-in.) The grader runs inside the secured, disposable box (candyboxing). Canonical definition in CLAUDE.md "Agent Driven Evaluation (ADE)"; the how-to + the worked example live in `/charly-eval:eval`.
