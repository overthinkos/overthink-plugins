...
their fragment directly from the view; the HOST-COUPLED `system-packages` (C1.2) + `builder` (C1.3) kinds
instead call back `HostBuild("step-emit", …)` (echoing the returned `EmitReply`) because their render needs
the host build engine (`system-packages`: DistroDef format templates; `builder`: the multi-stage
`buildStageContext` + `RenderTemplate` engine). Authoring + IR mechanism: `/charly-internals:install-plan` (the
