...
**Managed block is executor-based.** `EnsureManagedBlockVia(ctx, exec, shell, home, opts)` is the core writer for the env.d-sourcing block: `GetFile` the existing rc, merge the fenced block, `PutFile` it back. `VmDeployTarget` calls it with an `SSHExecutor` (guest fs); the external `local:` plugin writes the equivalent block out-of-process via the kit primitives (`ReplaceOrAppendManagedBlock`, `charly/plugin/kit/profile.go`). `EnsureManagedBlock(shell, home)` is a thin wrapper over the core writer with a local `ShellExecutor`. `home` is the DESTINATION user's home — the guest home for a VM deploy (see `/charly-internals:vm-deploy-target`).
