...
│                              # (referenced via charly.yml format_config)
...
| `config.go` | `charly.yml` parsing, inheritance resolution. `BuildFormats` type. `Distro` field. `ResolvedImage.Tags` (union). `SupportsTag()`, `SupportsBuild()` methods |
...
| `scaffold_project.go` | `new project` scaffolding + `charly.yml` mutation helpers (`ScaffoldProject`, `AddImage`, `AddLayerToImage`, `RemoveLayerFromImage`). All YAML round-trips go through the `yaml.v3` Node API so comments + key order are preserved. Tested in `scaffold_project_test.go`. |
