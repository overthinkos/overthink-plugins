...
The VM refactor introduced 12 new Go files + deleted 6 legacy structures in a single PR (see `/ov-internals:cutover-policy`). The new module topology:
...
**Deleted in the same PR** (see `/ov-internals:cutover-policy` for the rationale):

- `VmConfig` struct — from `ov/config.go`.
- `ImageConfig.Vm` + `ImageConfig.Libvirt` fields.
- `ResolvedImage.Vm`.
- `resolveVmConfig` function.
- `LabelVm` + `LabelLibvirt` label constants + `CapabilityLabelMap` entries (OCI label contract side — see `/ov-internals:capabilities`).
