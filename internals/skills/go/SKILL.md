...
| `scaffold_project.go` | `new project` scaffolding + `charly.yml` mutation helpers (`ScaffoldProject`, `AddBox`, `AddLayerToBox`, `RemoveLayerFromBox`). All YAML round-trips go through the `yaml.v3` Node API so comments + key order are preserved. Tested in `scaffold_project_test.go`. |
...
| `evalcollect.go` | `CollectEval(cfg, layers, imageName) *LabelEvalSet` walks the base-image chain — mirror of `CollectHooks` in `hooks.go:18-68` — with a visited-image guard so pathological cycles reported by `validateBoxDAG` can't hang the collector. Bucketizes checks into `layer`/`image`/`deploy` by source + scope, stamps `Origin` for reporting. `MergeDeployEval(baked, local)` implements id-based replace, append, and `{id: X, skip: true}` disable semantics. |
| `eval_cmd.go` | `EvalCmd` — the top-level `charly eval` command tree with three primary verbs (`Image EvalBoxCmd`, `Live EvalLiveCmd`, `Run EvalRunCmd`) plus 9 live-container probe sub-Cmds (cdp/wl/dbus/vnc/mcp/record/spice/libvirt/k8s) plus the eval-run management subcommands (list-ai/list-recipe/list-score/list/sync-credential/report/scope/last-tag/note/run-local/self-evaluate). `charly eval live` flow: `resolveContainer` → `containerImageRef` → `ExtractMetadata` → load `DeploymentNode.Eval` overlay → `MergeDeployEval` → `ResolveEvalVarsRuntime` → populate `Runner.Image`/`Instance` (so `cdp`/`wl`/`dbus`/`vnc` verbs can build CLI invocations) → `Runner.Run` → format results. `charly eval box` flow: `resolveLocalImageRef` (never reads charly.yml — see `local_image.go`) → `ExtractMetadata` → `ResolveEvalVarsBuild` → disposable `ImageExecutor` → run ONLY `candy`+`box` sections (deploy-scope skipped — for full-stack live eval use `charly eval live <name>`). |
...
Unmarshal-to-value + re-marshal scrambles comments, key order, and node styles. Every `charly.yml` editor in the authoring surface (`SetByDotPath` in `charly/yaml_setter.go`; `AddBox` / `AddLayerToBox` / `RemoveLayerFromBox` in `charly/scaffold_project.go`; `appendLayerPackages` in `charly/scaffold_cmds.go`) navigates `*yaml.Node` trees directly and only serializes with `yaml.Marshal(root)` at the very end. Tests (`charly/yaml_setter_test.go`, `charly/scaffold_project_test.go`) explicitly verify that leading file comments, sibling keys, and per-key inline comments all survive round trips.
