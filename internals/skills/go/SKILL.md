...
### Remote-layer resolver (`ov/refs.go` + `ov/layers.go`) — warn-and-newest-wins + reachability-scoped collection

Two cooperating pieces resolve `@github` layer refs to fetchable `(repo, version)` pairs. Both are version-aware through ONE shared mechanism, so direct and transitive references obey the same policy.

- **`LayerRef` (`ov/refs.go`)** — the single representation of a `require:` / `layer:` ref. It stores the ORIGINAL ref string (`Raw`, with any `@repo` prefix and `:version` suffix); `.Bare()` (the map-key form), `.Version()` (the pinned tag), and `.IsRemote()` are DERIVED. A `resolved` slot carries the qualified sibling key set by `qualifyRemoteSiblingDeps` after a remote layer is fetched, so ONE list serves both the graph (keys on `.Bare()`) and the transitive fetch (keys on the immutable `.Raw`). `Layer.Require` / `Layer.IncludedLayer` are `[]LayerRef` — there are no parallel bare/raw arrays.
- **`refVersionTracker` (`ov/refs.go`)** — enforces **warn-and-newest-wins**: a bare ref may be referenced many times; on a version DISAGREEMENT it warns once (naming both versions + their sources) and keeps the NEWEST (`compareSemver`, which orders CalVer correctly). `record(bareRef, version, source) (winning bool)` returns true when the version should be (re-)fetched (first sighting or newer than the current winner) and false when it loses. It backs BOTH `CollectRemoteRefsOpts` (initial collection) and the `ScanAllLayerWithConfigOpts` fix-point — so a transitive dep can never silently pull a different tag of an already-collected layer (the old version-blind `scanned map[string]bool` shipped images missing a newer pinned layer's packages — see `CHANGELOG.md`). The fix-point re-scans (overwrites the layer-map entry) when a newer winner appears (`scannedAt` tracks the materialized version).
...
`ov image reconcile` (see `/ov-build:reconcile`) is the operator tool that rewrites stale pins to the newest tag, clearing the warnings.
