...
The `ov image new` family groups three scaffolding verbs. See `/ov-image:image` for the full family overview and `/ov-image:layer` for the layer-authoring verb catalog it bootstraps.
...
4. `ov layer add-rpm my-svc openssh-server` — populate packages (see `/ov-image:layer`)
...
`ov image new layer <name>` writes into `layers/<name>/` relative to `os.Getwd()`. Override with `-C <dir>` / `--dir <dir>` / `OV_PROJECT_DIR=<dir>`. See `/ov-image:image` "Project directory resolution".
