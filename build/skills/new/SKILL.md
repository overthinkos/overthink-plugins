...
Each verb also auto-becomes an MCP tool (`box.new.project`, `box.new.box`, `box.new.candy`) via Kong reflection in `ov/mcp_server.go` — so an LLM agent driving `charly mcp serve` can scaffold a project from scratch over RPC. See `/charly-build:ov-mcp-cmd` "Authoring tools".
...
All six steps are also callable as MCP tools (`box.new.project`, `box.new.candy`, `candy.add-rpm`, …), so an agent driving `charly mcp serve` can run this entire flow over RPC. See `/charly-build:ov-mcp-cmd` "Authoring tools" for the worked MCP-only example.
...
`charly box new candy <name>` writes into `candy/<name>/` relative to `os.Getwd()`. Override with `-C <dir>` / `--dir <dir>` / `OV_PROJECT_DIR=<dir>`. See `/charly-image:image` "Project directory resolution".
...
- `/charly-build:ov-mcp-cmd` -- "Authoring tools" table + the MCP-only build-from-scratch worked example
