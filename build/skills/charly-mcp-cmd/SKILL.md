...
- `/charly-check:check` — parent router for the declarative `mcp:` check verb. The full `mcp` method allowlist (and how the verb relates to the in-core wl/dbus verbs and the out-of-process cdp/vnc/kube/spice/adb/appium plugin verbs) lives in its "Live-container verb catalog" + "Method allowlist — mcp" sections.
