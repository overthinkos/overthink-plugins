...
- `/ov-foundation:postgresql` — canonical `use_packaged:` entry with drop-in overrides
...
Resource caps (memory / cpus) are used by the chrome layer's crash-loop circuit breaker. See `/ov-selkies:chrome` and `/ov-foundation:supervisord` for the event-listener pattern.
...
- `/ov-foundation:supervisord` — Event listener pattern triggered by resource caps.
- `/ov-foundation:ov` — The ov-binary layer (composed by every ov-driving image). Paired with `/ov-coder:ov-mcp` which turns any image into an MCP server exposing the full ov CLI.
...
**Workflow position:** Pre-build. Author layers before adding them to images. See `/ov-build:image` (composition), `/ov-build:build` (building), `/ov-build:generate` (emission internals).
