...
|-----------|-------|---------|
| `ov eval cdp …` | `/ov-advanced:cdp` | Chrome DevTools Protocol — open/list/close tabs, click, type, eval, screenshot, axtree |
| `ov eval wl …` | `/ov-advanced:wl` | Wayland desktop input/windows/clipboard + sway IPC |
| `ov eval dbus …` | `/ov-advanced:dbus` | D-Bus calls, introspection, desktop notifications |
| `ov eval vnc …` | `/ov-advanced:vnc` | VNC framebuffer screenshot + click/key/type/passwd |
...
| `ov eval record …` | `/ov-advanced:record` | Recording sessions — start/stop/list/cmd. Terminal (asciinema) or desktop (pixelflux/wf-recorder). Container-only. |
| `ov eval spice …` | `/ov-advanced:spice` | SPICE wire client for VMs — handshake, native-SPICE framebuffer screenshot, input injection. VM-only. |
| `ov eval libvirt …` | `/ov-advanced:libvirt` | libvirt-RPC test commands for VMs — info, domain XML, QMP, qemu-guest-agent, snapshots, events. VM-only. |
...
- **Live-container probe verbs under `ov eval`** — `/ov-advanced:cdp`, `/ov-advanced:wl`, `/ov-advanced:dbus`, `/ov-advanced:vnc`, `/ov-build:mcp`, `/ov-advanced:record`, `/ov-advanced:spice`, `/ov-advanced:libvirt`, `/ov-advanced:eval-k8s` are dispatched as `ov eval cdp|wl|dbus|vnc|mcp|record|spice|libvirt|k8s`. See the Subcommands section above.
- `/ov-build:layer` — layer authoring; `eval:` field is part of every `layer.yml`.
- `/ov-build:image` — image-level `eval:` and `deploy_eval:` at composition time.
