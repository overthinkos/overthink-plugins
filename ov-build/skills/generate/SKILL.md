...
Invoked as `ov image generate`. See `/ov-build:image` for the family overview.
...
The generator is **config-driven** — distro format templates, builder stage templates, and init system fragments all come from a single `build.yml` (three top-level sections: `distro:`, `builder:`, `init:`) — and **declarative per-task** for install logic — each task verb (see `/ov-build:layer`) has a dedicated emitter that writes the right Containerfile directive.
...
4. tasks: iterated in author order:
   ...
   e. Adjacent same-verb same-user tasks coalesce into one directive
      (mkdir, link, setcap) — see /ov-build:layer execution-order section
...
The pivot is `ResolvedImage.UserAdopted`, set by the `user_policy:` reconciliation in `ov/config.go:ResolveImage`. See `/ov-build:image` "user_policy" for the policy semantics, `/ov-build:build` "base_user:" for the declaration, and `/ov-foundation:ubuntu` for the canonical adopt-mode worked example.
...
- `/ov-foundation:bootc-config` — bootc boot wiring that depends on the empty-init-stage fix (only `use_packaged:` entries, no custom-exec rendered bodies).
