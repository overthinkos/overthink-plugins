...
`/ov-marimo:osm-tools-layer`: shallow clone → `make -j$(nproc)` →
...
- `/ov-marimo:versatiles` — the tile-server service that auto-discovers
  `/workspace/tiles/shortbread/*.pmtiles` files
- `/ov-marimo:versatiles-style` — MapLibre style generator targeting
  the Shortbread schema (the notebook's shortbread cell renders the
  pipeline output using `colorful()`)
- `/ov-marimo:notebook-osm` — DAG cell 2 self-authors the
  `notebook_osm_shortbread_pipeline` that drives this layer
- `/ov-marimo:osm-tools-layer` — sibling tippecanoe pipeline + martin
