...
`/ov-marimo:marimo` so operators can inspect any of the four
...
The marimo image's OSM analytics pipelines write four sibling
...
- `/ov-marimo:osm-tools-layer` — companion martin tile server (the
  four sibling sources the viewer inspects)
- `/ov-marimo:notebook-osm` — DAGs producing the four PMTiles
  archives and the four MapLibre cells consuming them
- `/ov-marimo:maputnik-layer` — sibling static-SPA-via-http.server
