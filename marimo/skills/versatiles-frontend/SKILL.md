...
description: |
  VersaTiles Frontend — pre-built TypeScript SPA from versatiles-org/versatiles-frontend GitHub releases. Third static-SPA-via-python-http.server in the marimo image (after maputnik + pmtiles-viewer). Served on port 8002 (host-mapped 28002). Also re-exports the versatiles-style layer's bundle at /style/ so the notebook's mo.iframe can reach it via an absolute URL.
...
set) since the marimo image targets interactive exploration, not
minimal embedding. Build deps (`nodejs`, `npm`, `vite`) are NOT
needed inside the marimo image — saves ~250 MB image size.

This contrasts with `/ov-marimo:maputnik-layer` which DOES build
...
- `/ov-marimo:versatiles` — the tile-server backend the frontend
  connects to by default
- `/ov-marimo:versatiles-style` — the bundle re-exported under `/style/`
- `/ov-marimo:pmtiles-viewer` — sibling static-SPA layer (port 28001)
- `/ov-marimo:maputnik-layer` — sibling static-SPA layer (port 28000)
- `/ov-marimo:notebook-osm` — the shortbread MapLibre cell that loads
