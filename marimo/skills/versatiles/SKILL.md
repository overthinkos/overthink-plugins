...
crates inside the marimo image — wasteful when the upstream CI
already publishes a ready binary. Download pattern mirrors martin's
in `/ov-marimo:osm-tools-layer`: GitHub release API → asset URL
...
See `/ov-marimo:osm-tools-layer` "CRITICAL: martin caches pmtiles file
mtime at startup" — same restart-race pattern applies.

## Cross-references

- `/ov-marimo:marimo` — image composing this layer
- `/ov-marimo:shortbread` — produces the PMTiles files versatiles serves
- `/ov-marimo:versatiles-style` — MapLibre style generator paired with
  versatiles serve as tile backend
- `/ov-marimo:versatiles-frontend` — pre-built SPA on port 28002 for
  visually exploring tile content
- `/ov-marimo:osm-tools-layer` — sibling martin tile server (port
  23000); the two run in parallel for the comparison surface
- `/ov-marimo:notebook-osm` — the versatiles convert round-trip demo
