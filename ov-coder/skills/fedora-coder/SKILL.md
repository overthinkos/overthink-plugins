...
The everyday-development counterpart to `/ov-foundation:fedora-ov`: all the
...
`/ov-foundation:container-nesting`. Rootless-first by design (shares this
...
see `/ov-foundation:container-nesting` for the `mount_too_revealing()` +
...
`strip_components: 1` modifier. See `/ov-coder:uv` and `/ov-build:layer`.
...
# (see /ov-build:eval "Live vs disposable executor selection")
ov eval image ghcr.io/overthinkos/fedora-coder:latest
...
google-cloud-npm) by forking image.yml. See `/ov-build:image` for authoring.
...
- `/ov-foundation:container-nesting` — rootless nested podman recipe (authoritative RCA for `mount_too_revealing()` + `unmask=/proc/*`)
...
All four produce the same daily-dev surface (sshd on 2222, ov-mcp on 18765, 5 AI CLIs, full language runtimes, DevOps tooling). Pick based on distro-family alignment with your team/CI. See `/ov-build:image` "user_policy" for the adopt-vs-create reconciliation that gives ubuntu-coder its `ubuntu` username.
...
- `/ov-foundation:fedora-ov` — minimal ov toolchain (no coding CLIs, no DevOps), also uid=1000 with sudo.
...
- `/ov-build:eval` — live-service tests (`ov eval live <name>`) and build-scope tests (`ov eval image <ref>`)
- `/ov-build:mcp` — MCP gateway documentation; `--no-default-repo` to disable auto-fallback
- `/ov-advanced:vm` — nested libvirt VMs (via virtqemud inside the container)
- `/ov-build:layer` — authoring reference (covers the new `strip_components:` modifier)
...
- Understanding the rootless-first architectural pattern shared by the
  ...
  `/ov-foundation:container-nesting`; the composition that proves it works
