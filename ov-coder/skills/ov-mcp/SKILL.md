...
`/ov-coder:fedora-coder`, `/ov-foundation:fedora-ov`).
...
- `/ov-foundation:ov` — The underlying binary layer this wraps.
- `/ov-foundation:supervisord` — Init system for the `ov-mcp` program.
...
- `/ov-build:image` — "Project directory resolution" covers the `-C` / `--dir` / `OV_PROJECT_DIR` global flag and `--repo` / `OV_PROJECT_REPO`.
- `/ov-core:config` — `--bind project=<path>` is the deployer's handshake with this layer's `volumes:` declaration.
- `/ov-build:eval` — Deploy-scope `mcp:` test verb methods used here.
