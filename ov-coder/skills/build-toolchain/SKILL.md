...
## Related Layers
- `/ov-foundation:pixi` — Sibling in builder images for conda-forge package builds
- `/ov-coder:nodejs` — Sibling in builder images for npm package builds
- `/ov-foundation:yay` — Pairs in archlinux-builder for AUR builds
- `/ov-coder:rust` — Disambiguates the builder-stage `rust`+`cargo` packages here from the runtime `rust` layer
- `/ov-foundation:rpmfusion` — Must be applied **before** this layer in fedora-builder so codec dev libs (`x264-devel`, `ffmpeg-devel`) can install
