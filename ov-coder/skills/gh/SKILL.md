...
- `/ov-coder:arch-ov`, `/ov-foundation:fedora-ov`, `/ov-coder:fedora-coder` — power-user images that compose gh explicitly
...
- `/ov-foundation:agent-forwarding` — pairs with gh for SSH/GPG agent access (you usually want both when driving gh from inside a container with the host's GPG keys forwarded)
- `/ov-foundation:github-runner` — self-hosted Actions runner; different layer, different purpose
