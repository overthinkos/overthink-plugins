...
- `/ov-foundation:pixi` — direct-download pattern this layer now mirrors. Pixi remains a legitimate multi-binary Python env manager for images that genuinely need one (jupyter, whisper, openwebui); uv is too small to justify the overhead.
- `/ov-coder:typst` — sibling direct-download binary pattern.
- `/ov-foundation:python` — the pixi-python meta-layer this layer **no longer** depends on.
...
- `/ov-build:layer` — authoring reference (covers `download:` verb + `strip_components:` modifier)
