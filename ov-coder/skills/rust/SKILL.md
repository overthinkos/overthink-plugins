...
| Rustc/cargo at **build time only**, for compiling a cdylib that's copied into the final image (e.g., pixelflux_wayland) | Already provided by `/ov-foundation:fedora-builder` via `/ov-coder:build-toolchain`'s system `rust`+`cargo` packages — no extra layer needed |
