...
The `ov image new` family groups three scaffolding verbs. See `/ov:image` for the full family overview and `/ov:layer` for the layer-authoring verb catalog it bootstraps.
...
Each verb also auto-becomes an MCP tool (`image.new.project`, `image.new.image`, `image.new.layer`) via Kong reflection in `ov/mcp_server.go` — so an LLM agent driving `ov mcp serve` can scaffold a project from scratch over RPC. See `/ov:mcp` "Authoring tools".
...
Follow up with `ov layer add-rpm sshd openssh-server openssh-clients` (see `/ov:layer`) to populate packages without manually editing YAML. The `ov layer add-rpm` helper handles the scaffold's null `packages:` → sequence upgrade automatically.
...
4. `ov layer add-rpm my-svc openssh-server` — populate packages (see `/ov:layer`)
...
All six steps are also callable as MCP tools (`image.new.project`, `image.new.layer`, `layer.add-rpm`, …), so an agent driving `ov mcp serve` can run this entire flow over RPC. See `/ov:mcp` "Authoring tools" for the worked MCP-only example.
...
- `/ov:layer` -- Layer authoring guide, layer.yml format, install files + the `ov layer set / add-rpm / add-deb / add-pac / add-aur` editing surface
- `/ov:mcp` -- "Authoring tools" table + the MCP-only build-from-scratch worked example
