...
**Critical:** The `openclaw models auth login` TUI requires a real terminal to complete the post-callback token exchange. Do NOT pipe through `tee` or redirect stdout — it breaks the TUI event loop. **Use `ov tmux`** (see `/ov-advanced:tmux`):
...
For browser-assisted OAuth (Google sign-in), see `/ov-advanced:cdp`.
...
- `/ov-advanced:cdp` -- `ov eval cdp` CDP commands (lower-level container-external automation)
...
- `/ov-advanced:vnc` -- VNC desktop automation and password management
- `/ov-advanced:alias` -- Host command aliases (`ov alias add openclaw`)
