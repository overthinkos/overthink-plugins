...
- `/ov-advanced:cdp` — Chrome DevTools Protocol (used with tmux for OAuth flows)
- `/ov-advanced:openclaw` — OpenClaw gateway config (OAuth requires tmux for token exchange)
- `/ov-core:service` — Supervisord service management (different scope: persistent services vs ad-hoc commands)
- `/ov-foundation:tmux` — The tmux layer definition
- `/ov-foundation:dbus` — D-Bus session bus (required for notifications)
