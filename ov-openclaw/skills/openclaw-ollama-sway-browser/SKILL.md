...
- `/ov-openclaw:openclaw-full-ml` — metalayer: openclaw-full + whisper + sherpa-onnx
- `/ov-ollama:ollama` — LLM inference server
- `/ov-selkies:sway-desktop` — full desktop composition
- `/ov-foundation:cuda` — GPU support (via nvidia base)

## Related Images

- `/ov-openclaw:openclaw-sway-browser` — same tools but no GPU/ML (lighter)
- `/ov-openclaw:openclaw-ollama` — headless OpenClaw + Ollama (no desktop, no tools)
- `/ov-openclaw:openclaw` — minimal headless gateway
...
**Critical:** The `openclaw models auth login` TUI requires a real terminal. Running it piped through `tee` or with stdout redirected breaks the post-callback token exchange. **Use `ov tmux`** (see `/ov-advanced:tmux`):
...
**Why `ov tmux`:** The `openclaw models auth login` TUI needs an active terminal to process the OAuth callback. The callback server (port 1455) receives the authorization code and displays "Authentication successful" in the browser, but the token exchange with OpenAI's servers requires the TUI event loop to be responsive. `ov shell --tty` uses `script(1)` which breaks when piped/backgrounded. `ov tmux run` provides a real tmux terminal. See `/ov-advanced:tmux` for full documentation.
...
See `/ov-advanced:openclaw` for full gateway configuration reference.
