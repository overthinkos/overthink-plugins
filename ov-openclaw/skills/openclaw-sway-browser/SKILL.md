...
- `/ov-openclaw:openclaw-full` — metalayer composing openclaw + chrome + 26 tool layers
- `/ov-selkies:sway-desktop` — full desktop (pipewire, wayvnc, chrome-sway, terminal, file manager, waybar)

## Related Images

- `/ov-openclaw:openclaw` — headless gateway only (minimal)
- `/ov-openclaw:openclaw-ollama-sway-browser` — adds CUDA, Ollama, Whisper, sherpa-onnx for ML
...
**Key points:**
- Use `ov tmux run` for the OAuth TUI (not `--tty` piped through `tee`) — see `/ov-advanced:tmux`
- Click "Continue with Google" then "Continue" on consent using `ov eval cdp click --vnc`
