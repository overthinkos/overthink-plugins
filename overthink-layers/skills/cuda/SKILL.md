...
- `/overthink-images:nvidia` (base for all GPU images)
- `/overthink-images:immich-cuda`

## Related Layers

- `/overthink-layers:python-ml` -- ML Python environment (depends on cuda)
- `/overthink-layers:jupyter` -- Jupyter notebooks (depends on cuda)
- `/overthink-layers:ollama` -- LLM server (depends on cuda)
- `/overthink-layers:comfyui` -- image generation (depends on cuda)
