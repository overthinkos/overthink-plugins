...
- `/overthink-layers:python` -- Python runtime dependency
- `/overthink-layers:traefik` -- reverse proxy (depends on supervisord)
- `/overthink-layers:dbus` -- D-Bus session bus (depends on supervisord)
- `/overthink-layers:ollama` -- LLM server (depends on supervisord)
- `/overthink-layers:openclaw` -- AI gateway (depends on supervisord)
- `/overthink-layers:postgresql` -- database (service layer)
- `/overthink-layers:redis` -- cache (service layer)
