...
|----------|-------|
| Dependencies | `nodejs` |
| Install files | `layer.yml`, `pixi.toml`, `package.json`, `user.yml` |
...
- `/overthink-layers:nodejs` -- required dependency (provides npm for markdownlint-cli)
- `/overthink-layers:pixi` -- transitive (provides pixi for pre-commit installation)
