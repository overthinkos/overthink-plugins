...
The wrapper script in `/ov-versa:airflow-layer` does three things
...
```bash
ov eval mcp call versa fetch_dags '{"args":{}}' --name airflow
```

Trigger a DAG:

```bash
ov eval mcp call versa post_dag_run '{"args":{
...
direct REST API path used in `/ov-versa:notebook-osm` (which
...
- `/ov-versa:airflow-layer` — layer that runs the server
- `/ov-versa:versa-mcp` — the OTHER MCP server in the same pod
- `/ov-versa:notebook-osm` — uses direct REST instead of MCP for triggering
- `/ov-versa:versa` — image composing the airflow layer
