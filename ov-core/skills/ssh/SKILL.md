...
- `/ov-advanced:spice` — `--uri` + `--socket` flags on `ov eval spice`.
- `/ov-advanced:libvirt` — `--uri` flag on every `ov eval libvirt` verb.
- `/ov-advanced:vnc` — `ov eval vnc vm <name> …` subcommand group.
