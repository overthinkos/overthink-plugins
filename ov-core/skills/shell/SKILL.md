...
**Shared code path:** `ov shell` calls `appendAutoDetectedEnv()` in `ov/devices.go` — the same function used by `ov config` and `ov start`. That means the three commands produce an identical env set on every run, eliminating drift that used to exist when DRINODE injection was scattered across 10 different call sites before commit `8f6f322`. See `/ov-core:doctor` (Hardware Detection) for the probe side, `/ov-foundation:nvidia` (DRINODE Auto-Injection) for the NVIDIA consumer, and `/ov-foundation:rocm` (Runtime Environment) for the AMD consumer.
...
- `/ov-build:pull` -- **Required** before `ov shell` can work on a fresh host. Fetches the image into local storage so `ExtractMetadata` can read its OCI labels. Handles remote refs (`@github.com/...`) that `ov shell` itself rejects.

### Deploy-mode neighbors

- `/ov-core:cmd` -- Single command execution with D-Bus notification (running containers only)
- `/ov-advanced:tmux` -- Persistent tmux sessions (survives disconnects, needed for TTY-dependent TUI programs)
...
- `/ov-advanced:cdp` -- Chrome DevTools Protocol automation
- `/ov-advanced:wl` (sway subgroup) -- Sway compositor control
...
- `/ov-build:image` -- Image definitions (ports, volumes, env) in `image.yml`; authoritative source before a pull
- `/ov-build:build` -- Build the image you intend to shell into

### Layer skills

- `/ov-foundation:nvidia`, `/ov-foundation:rocm` -- GPU layers consuming the auto-injected env
