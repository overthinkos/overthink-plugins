...
- `/ov-advanced:enc` -- Encrypted storage (mounted inline by ov start)
- `/ov-core:config` -- `run_mode`, `auto_enable`, `engine.run` settings
- `/ov-advanced:cdp` -- CDP status subcommand (`ov eval cdp status`)
- `/ov-advanced:vnc` -- VNC status subcommand (`ov eval vnc status`)
- `/ov-advanced:wl` -- Desktop automation + sway subgroup (`ov eval wl sway status`)
- `/ov-advanced:wl` -- WL status subcommand (`ov eval wl status`)
