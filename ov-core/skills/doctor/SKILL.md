...
- `/ov-advanced:udev` — install udev rules for GPU device access
...
- `/ov-advanced:enc` — credential lookup path behind the Secret Service collection + keyring-index checks; iteration-capable ssClient; broken-collection troubleshooting
...
- `/ov-foundation:nvidia` — NVIDIA GPU runtime support + DRINODE Auto-Injection section
- `/ov-foundation:rocm` — AMD ROCm runtime support + DRINODE/HSA_OVERRIDE_GFX_VERSION auto-detect table
