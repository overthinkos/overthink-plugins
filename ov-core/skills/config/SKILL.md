...
The `OV_PROJECT_DIR` env var is consumed by the ov binary's global `-C` / `--dir` / `OV_PROJECT_DIR` flag (`ov/main.go` calls `os.Chdir(Dir)` before Kong dispatch). See `/ov-build:image` "Project directory resolution" and `/ov-build:mcp` "Deployment: the `ov-mcp` layer" for the full pattern.
...
the credential in a sibling healthy collection. See `/ov-advanced:enc` for the full
...
See `/ov-build:layer` for `env_provides` field documentation.
...
Parallel to `env_requires` but for credential-backed env vars. Layers declare `secret_requires` in `layer.yml` (see `/ov-build:layer`). `ov config` resolves each entry from the credential store via `ResolveCredential` and, if any required value is not stored, aborts with a clear remediation message:
...
See `/ov-advanced:sidecar` for full sidecar documentation.
...
- `/ov-advanced:sidecar` — Sidecar containers, pod networking, Tailscale exit nodes, Environment Contract (how sidecars participate in provides filtering)
...
- `/ov-advanced:enc` — Encrypted storage details
...
- `/ov-build:layer` — Volume, secret, `env_provides` / `env_requires` / `env_accepts` declarations, security resource cap fields, `service:` blocks
- `/ov-build:image` — Image composition, inheritance, OCI label emission, tunnel deploy.yml-only note (`labels.go:238`)
...
- `/ov-foundation:supervisord` — Event listener pattern that pairs with the resource caps
- `/ov-foundation:nvidia`, `/ov-foundation:rocm` — GPU layers that consume DRINODE auto-injection
