...
- `/ov-distros:archlinux-builder` — adds pixi, nodejs, build-toolchain, yay
- `/ov-distros:arch-test` — pacman + AUR packaging test
- `/ov-coder:arch-ov` — full ov toolchain on Arch

## Multi-Distro Support

This is the Arch counterpart to `/ov-distros:fedora`. The tag system (`distro: [archlinux]`, `build: [pac]`) selects `pac:` package sections and `pac:` tasks in tasks:. Layers shared between Arch and Fedora images use distro-specific sections:
