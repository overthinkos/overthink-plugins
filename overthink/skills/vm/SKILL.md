...
- `/overthink:build` -- Building container images before VM disk builds
- `/overthink:config` -- VM default settings (`vm.*` config keys)
- `/overthink:image` -- `bootc`, `vm:`, and `libvirt` fields in images.yml
- `/overthink:layer` -- `libvirt` field in layer.yml
