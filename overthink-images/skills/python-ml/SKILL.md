...
- `/overthink-layers:python-ml` — ML Python packages via pixi, llama.cpp
- `/overthink-layers:cuda` — GPU support (via nvidia base)

## Related Images

- `/overthink-images:nvidia` — parent (GPU without ML packages)
- `/overthink-images:jupyter` — adds Jupyter notebook server on top of ML stack
