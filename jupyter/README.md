...
No skills of its own — the backing documentation lives in `/ov-jupyter:jupyter-mcp` (server implementation) and the three image skills that publish the server.
...
- `/ov-jupyter:jupyter-mcp` — the MCP server implementation (11 tools, FastMCP + Tornado-ASGI bridge, CRDT integration, auto-attach + canonicalization design principles)
- `/ov-jupyter:jupyter` — lightweight parent layer (composes `jupyter-mcp`)
- `/ov-jupyter:jupyter-ml` — GPU ML parent layer (composes `llama-cpp`, `unsloth`, `jupyter-mcp`)
