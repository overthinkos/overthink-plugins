...
- `/ov-marimo:marimo-layer` — layer that runs the server
- `/ov-marimo:airflow-mcp` — the OTHER MCP server in the same pod
- `/ov-marimo:notebook-osm` — example notebook diagnosed via this MCP
- `/ov-build:mcp` — MCP probe verb authoring + URL rewriter
- `/ov-build:eval` — `ov eval mcp` subcommand reference
