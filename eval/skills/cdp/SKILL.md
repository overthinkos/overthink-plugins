...
Key enablers:
- `ov tmux run` provides a real terminal for the TUI to complete the token exchange (see `/ov-automation:tmux`)
...
- `/ov-eval:eval` -- parent router; `ov eval cdp …` is how every invocation is dispatched.
- `/ov-eval:wl` -- Wayland desktop automation (sibling verb under `ov eval`); also sway subgroup for compositor control.
- `/ov-eval:vnc` -- VNC desktop automation (sibling verb; same container, pixel-level interaction).
- `/ov-eval:dbus` -- D-Bus calls and notifications (sibling verb under `ov eval`).
...
**Workflow position:** Desktop automation. Use after a desktop container is running. Preferred over VNC for structured interaction. See also `/ov-eval:vnc` (pixel), `/ov-eval:wl` (sway subgroup) (window).
