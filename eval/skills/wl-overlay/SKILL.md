...
description: |
  ...
  MUST be invoked before any work involving: charly eval wl overlay commands, recording
...
`charly eval wl overlay` creates and manages fullscreen Wayland overlays using the layer-shell protocol via gtk4-layer-shell. Overlays render directly in the compositor, appearing natively in screen recordings without post-production editing. Supports true alpha transparency — desktop content is visible through semi-transparent overlays.
...
|--------|---------|-------------|
| Show overlay | `charly eval wl overlay show <image> --type text --text "Hello"` | Display a named overlay |
| Hide one | `charly eval wl overlay hide <image> --name intro` | Remove specific overlay |
| Hide all | `charly eval wl overlay hide <image> --all` | Remove all overlays |
| List | `charly eval wl overlay list <image>` | List active overlays (JSON) |
| Status | `charly eval wl overlay status <image>` | Check daemon health |
...
```bash
charly eval wl overlay show myimage --type text \
...
```bash
charly eval wl overlay show myimage --type lower-third \
...
```bash
charly eval wl overlay show myimage --type watermark \
...
```bash
charly eval wl overlay show myimage --type countdown --seconds 5 --name cd
```
...
```bash
charly eval wl overlay show myimage --type highlight \
...
```bash
charly eval wl overlay show myimage --type fade --color black --name outro
```
...
```bash
charly eval wl overlay show myimage --type text --text "INTRO" --duration 5s --name intro
```
...
Overlays compose naturally with `charly eval record`:

```bash
# Title card
charly eval wl overlay show myimage --type text --text "Building a REST API" \
  --bg "rgba(0,0,0,0.9)" --font-size 64 --name title

# Start recording
charly eval record start myimage -n demo -m desktop

# Fade out title after 3s
sleep 3
charly eval wl overlay hide myimage --name title
...
# Stop recording
charly eval record stop myimage -n demo -o demo.mp4
charly eval wl overlay hide myimage --all
```
...
- `/charly-eval:wl` — Compositor-agnostic desktop automation (screenshots, input, windows)
- `/charly-eval:record` — Terminal and desktop recording (overlays compose with recording workflow)
- `/charly-eval:cdp` — Chrome DevTools Protocol (DOM-level interaction)
