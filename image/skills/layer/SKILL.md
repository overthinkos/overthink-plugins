...
| `localpkg` | string (dir) | **Arch/CachyOS deploy package format** (sibling of `apk`, for pac targets). A directory ref to a bundled PKGBUILD (e.g. `pkg/arch`). Compiles to a `LocalPkgInstallStep` (`/ov-internals:install-plan`) emitted at "step 2.5", BEFORE the layer's `task:`. On a pac deploy target (`target: local` on a pac host / `target: vm` into a pac guest) the HOST builds the PKGBUILD via `makepkg`, **resolves the built package's builder-resolvable dependency closure** (its `.PKGINFO depends` ∩ host-foreign packages = the AUR-only deps), builds that closure via the `aur` builder, and `pacman -U`'s the package + its closure onto the target — so the install succeeds even when those AUR deps aren't pre-present (no reliance on another layer having installed them first). Every command is rendered from the distro's `format.<fmt>.local_pkg:` block in `build.yml` (no hardcoded literals). Resolution walks up from the deploy project dir, so a consumer under `image/<distro>` finds the superproject's `pkg/arch`. Skipped at image build + on non-pac targets — the layer's own `task:` (curl/COPY) is the fallback there. The `ov` layer uses this to install `ov` as the `overthink-git` package at `/usr/bin/ov` on Arch deploys instead of a curl'd binary. |
