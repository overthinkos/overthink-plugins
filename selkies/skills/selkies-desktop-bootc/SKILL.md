...
2. `/ov-distros:agent-forwarding` — SSH/GPG agent socket plumbing
3. `/ov-distros:bootc-base` — `sshd` + `qemu-guest-agent` + `/ov-distros:bootc-config`
4. `/ov-distros:rpmfusion` — free + nonfree repos (required for ffmpeg in selkies-desktop)
...
6. `/ov-infrastructure:tailscale` — systemd-enabled tailscaled
7. `/ov-infrastructure:keepassxc` — GUI password manager
8. `/ov-infrastructure:dbus-layer` — session bus
9. `/ov-tools:ov` — ov CLI in-image
...
- `/ov-infrastructure:tailscale` — mesh VPN (this image's standalone tailscaled)
- `/ov-infrastructure:keepassxc` — password manager
- `/ov-distros:bootc-base` — sshd + guest-agent + bootc-config
- `/ov-distros:bootc-config` — bootc boot wiring (tty1 autologin, graphical target, systemd-user supervisord)
- `/ov-distros:rpmfusion` — the nonfree-codec layer needed by selkies-desktop/ffmpeg
- `/ov-vm:vm` — VM build + lifecycle + engine.rootful modes + the `/dev:/dev` mount explanation
...
- `/ov-image:image` — the `distro:` field semantics + external-base caveat
- `/ov-eval:eval` — declarative testing (including the USER-context gotcha for dual-mode images)
...
- Understanding the interaction between `/ov-distros:bootc-config` and `/ov-infrastructure:supervisord` on bootc
