...
All overlay types render with true RGBA compositor transparency. See `/ov-eval:wl-overlay` for the full recording workflow.
...
- `/ov-selkies:sway-desktop-vnc`, `/ov-selkies:sway`, `/ov-selkies:wayvnc`,
  `/ov-selkies:chrome-sway`, `/ov-selkies:xdg-portal`, `/ov-infrastructure:dbus-layer`,
  `/ov-tools:ov`, `/ov-distros:agent-forwarding`
- `/ov-eval:eval` — declarative testing framework (parent router for `ov eval cdp|wl|dbus|vnc|mcp`)
- `/ov-eval:vnc` — VNC automation on this image
