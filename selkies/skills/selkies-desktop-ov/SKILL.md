...
surgical, documented in `/ov-distros:container-nesting` with the full
kernel-level RCA.
...
See `/ov-distros:container-nesting` for the full kernel-level RCA. The
short version:
...
See `/ov-infrastructure:virtualization` for the full story. The short version:
...
`unmask=/proc/*` fix lives in `/ov-distros:container-nesting`. The plan
...
- `/ov-distros:container-nesting` — the rootless nested podman recipe (authoritative RCA for `mount_too_revealing()` + `unmask=/proc/*`)
- `/ov-infrastructure:virtualization` — QEMU/KVM packages + supervisord-managed virtqemud/virtnetworkd for rootless session libvirt
- `/ov-distros:nvidia` — GPU runtime (via the `nvidia` base image)
- `/ov-distros:cuda` — CUDA toolkit (via the `nvidia` base image)
...
- `/ov-distros:nvidia` — the parent base image (CUDA + NVIDIA runtime).
...
- Nested rootless podman or rootless libvirt VMs from a non-root
  container (the kernel-level RCA is in `/ov-distros:container-nesting`
  and `/ov-infrastructure:virtualization`; the composition that puts them
  together is here).
