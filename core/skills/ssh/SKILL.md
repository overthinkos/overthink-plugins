...
- `/ov-vm:arch` — "Connecting from a remote workstation" —
  the canonical worked example across all three paths.
- `/ov-build:settings` — `hosts.<alias>` key schema.
- `/ov-eval:spice` — `--uri` + `--socket` flags on `ov eval spice`.
- `/ov-eval:libvirt` — `--uri` flag on every `ov eval libvirt` verb.
- `/ov-eval:vnc` — `ov eval vnc vm <name> …` subcommand group.
